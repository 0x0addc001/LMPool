...
- `--foreground-transfer-fixed-latency-ms`: fixed per-plan loaded-serving
  residual outside the idle data-path profile. The paper default is 40 ms,
  calibrated from target dispatch-to-publication timing in an independent
  pilot; it includes scheduling, NCCL queuing, target publication, and block
  registration.

...
| 批量 NVLink transfer | 将多层、多 block KV 合并为连续 payload；使用 NVLink pair 专属 P2P process group，预热 communicator，并移除 pair 操作中的全局 barrier | 避免六卡全局同步，把小粒度通信摊销到 4/8-block 批次 |
...
图 1：左图为 routing workload 的初始 prompt token 复用率；中图为 2 个模型、3 对 NVLink 上的平均带宽及 min-max；右图为 LMPool 相对 `multi-gpu` 在 session handoff 中的改进，时延项均按“下降比例”表示。
...
六组“模型 x NVLink pair”测试全部通过搬运前后 KV 一致性校验。带宽随 batch 增大而提高，证明连续 payload 和 pair-local 通信能有效摊薄固定开销；线上成本模型应采用对应 batch 的实测带宽，而不是链路标称值。
...
下面四张图由论文实验目录中的 benchmark 脚本直接生成，展示五次重复实验聚合后的绝对吞吐、goodput、时延、prefix reuse 和 GPU 利用率。它们与图 1 的结论性摘要互补，不经过相对增益换算。

![Routing 与 session-handoff 绝对指标](./figures/report_20260720_absolute_metrics.png)

图 2：论文主结论对应的绝对 throughput 和 mean TTFT。该图保留原始单位，直接对比 round-robin、transfer-only 和 LMPool，避免仅依赖相对增益。

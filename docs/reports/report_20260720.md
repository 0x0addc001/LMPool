...
Routing 将未复用 prefill token 减少约 **49.6%/50.5%**，TTFT 降低 **20.2%/10.6%**，同时吞吐提升 **2.7%/2.2%**。这说明当前路由没有通过把请求集中到少数 GPU 来换取命中率，而是在控制负载倾斜的同时将 locality 收益转化为实际计算节省。
...
- CPU 测试结果为 **183 passed, 1 skipped**；硬件 microbenchmark 对所有 NVLink pair 完成数据一致性校验。

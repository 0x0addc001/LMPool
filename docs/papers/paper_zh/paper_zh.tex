...
对被拆分为完整 block $b_0,\ldots,b_k$ 的 prompt，\sys{} 计算 chained hash：
\begin{equation}
...
partner；不相关的 PCIe 或 NUMA peer 不作为 transfer path。原型补充而非替代 RDMA-based
...
\begin{figure*}[t]
  ...
  control plane 和 per-GPU data plane。每个详细 GPU rank 使用相同的 $2\times2$ 布局，
  重复 rank pair 表示其他实例。Global Scheduler 向每个 Local Scheduler 发送 routing 和
  transfer command；每个 Local Block Manager 向 Global Block Manager 发布版本化 block 与
  load snapshot。只有配对的 Model Runner 通过绿色 NVLink path 交换打包 KV tensor，物理
...
发送版本化 snapshot。没有 Python BlockManager instance 在进程间共享。
...
snapshot 重建 control metadata；当前原型不能在 launcher failure 后继续服务。
...
\sys{} 使用 Python 与 PyTorch 扩展 Mini-vLLM。表~\ref{tab:modules} 总结 implementation boundary。
遗留函数名 swap\_out 与 swap\_in 仍在 KV-transfer module 内；论文与公开文档使用 transfer，
因为 data path 同时支持 copy 与 move，且没有 CPU swap tier。
...
仓库为每个 engine module 提供 focused test，而非仅依赖 benchmark 成功完成。当前 CPU test run 包含
190 个通过测试和一个被跳过的 hardware-gated test。表~\ref{tab:tests} 总结 coverage。
...
测试使用 fake component 获得 deterministic control decision。CUDA/NCCL case 有明确 marker，使 CPU-only
CI 仍可用。Hardware test 将已知 KV value 从 rank~0 发送到 rank~1，验证 transfer 后 equality，并以
completion timeout 检测不一致的 send/receive ordering。
...
path、pair 间无 direct path。NV4 表示四条 bonded link，而非 fourth-generation NVLink。RTX 3090
...
artifact 对应 paper batch `20260727T231622Z`。
...
对 $R$ 次完整重复测得的 scalar metric，报告 sample mean

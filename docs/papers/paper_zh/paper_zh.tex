...
又可能使该 GPU 过载。本文提出 \sys{}，一个多 GPU LLM serving 系统：它以 KV-aware
...
端到端实验验证了 background placement 以及 foreground offloading 的功能，
...
\sys{} 使用 Python 与 PyTorch 扩展单 GPU 推理引擎 Mini-vLLM。表~\ref{tab:modules} 总结了 implementation。
...
评估回答四个问题：(Q1) KV-aware routing 是否在长共享前缀下改善 service？
(Q2) Transfer data path 提供何种 latency/bandwidth？(Q3) Background placement 是否
在 load skew 下执行，它带来什么 serving trade-off？(Q4) Foreground offloading 是否能在 memory skew
下安全且高效地工作？Q1 是主要端到端性能主张，Q2-Q4 证明 data-path correctness、protocol
...
Routing & 192 & 16 个 recurring group，每组首请求为 cold，共享前缀长度为 1x、3x、5x & 365,880 / 1,084,728 / 1,803,576 & 91.67\% & 86.20\% / 91.38\% / 89.93\% & 16 & 测试 locality-aware routing。\\
Load skew & 192 & 三个长 hot group，三个 warm-up 后为 189 次 hot reuse & 1,444,344 & 98.44\% & 97.15\% & 96 & 测试 background placement。\\
Memory skew & 72 & 六个 session warm-up、30 个 anchor/cold pressure request、36 个 session reuse & 486,554 & 91.67\% & 72.29\% & 48 & 测试 foreground offloading。\\\bottomrule
...
\begin{figure*}[t]\centering\includegraphics[width=\textwidth]{fig_suite_routing.png}
\caption{随共享前缀长度增加的 Routing 结果}\label{fig:suite-routing}\end{figure*}
\begin{figure*}[t]\centering\includegraphics[width=\textwidth]{fig_suite_skew.png}
\caption{Load-skew 与 Memory-skew 工作负载上的 Throughput 和 Mean TTFT}\label{fig:suite-skew}\end{figure*}
\begin{figure*}[t]\centering\includegraphics[width=0.92\textwidth]{fig_suite_transfer_profile.png}
\caption{跨 Direct GPU Pair 的打包 NVLink KV Transfer Profile}\label{fig:suite-transfer-profile}\end{figure*}

\subsection{Q1：KV-aware routing 是否在长共享前缀下改善 service？}
...
\subsection{Q2：Transfer data path 提供何种 latency/bandwidth？}
...
\subsection{Q3：Background placement 是否
在 load skew 下执行，它带来什么 serving trade-off？}
...
\subsection{Q4：Foreground offloading 是否能在 memory skew
下安全且高效地工作？}

\paragraph{Offloading 被验证功能，但未必可以仅凭自身提高性能}
Memory skew 中，0.6B full-system 结果报告 35 个释放的
...
与 routing-only 在统计上接近。
1.7B 结果记录了 moved block，但不满足完整 offload
verification predicate，其 full-system throughput/latency 与 routing-only 接近，confidence interval
重叠。
该实验验证安全的 capacity-release transaction，但未证明增量 throughput
收益。
因此，memory skew 被用于 implementation/safety result，而非证明 transfer-out 改善 serving
...
NVLink fabric 改变 candidate selection。
NVSwitch fabric 需要单独评估：它扩大每个 source GPU 的可行 target set，且并发 transfer 会争用共享 fabric，
而非独立的 pair-local link。因此，在当前拓扑上测得的 pair-specific profile 不能证明 all-to-all NVSwitch domain
上的 routing 或 admission behavior。
结果不应直接外推到 RDMA 或 PCIe-only cluster。

内部消融提供受控 mechanism comparison，但不比较完整 production serving stack 在 production traces 上。Kernel、scheduler、
prefix index、connector implementation 和 workload 的差异使这些改善不能被解释为相对 vLLM、SGLang、Mooncake 或
...
未来应在相同 model、arrival process 与 KV budget 下用 production traces 比较 production serving engine，
并改变 model size、KV geometry、block size 与 GPU count。
还应在 eight-GPU NVSwitch domain 上评估，包括 concurrent transfer profile、较大 target set 上的 topology-aware
candidate selection，以及 shared-fabric contention 下的 end-to-end scaling。
Transfer study 应报告 held-out prediction error，并对 piecewise-linear profile、EWMA correction、safety ratio 与
no-gate policy 做 ablation。Production deployment 还需要 replicated control metadata 与 launcher failover，而非当前
single-control-process recovery model。
...
\subsection{Cluster 编排}

NVIDIA Dynamo 是 datacenter-scale inference framework，结合 disaggregated serving、基于 worker load 与
KV-cache overlap 的 KV-aware routing，以及通过 KV Block Manager 实现的 tiered KV offload~\citep{NVIDIADynamo}。
其 router 和 cache event 支持 cluster-level worker selection，storage hierarchy 将 KV capacity 扩展到 GPU HBM
之外。\sys{} 解决更窄的 scale-up 问题：在由 configured direct NVLink pair 连接的 data-parallel replica 间协调物理
局部 HBM cache。\sys{} 的重点是根据 pair 与 payload 的 cost 对 complete KV block chain 的 transactional copy/move
准入，并以 placement lease 服务后续 request。二者是互补而非可相互替代的设计，本文也不比较其 end-to-end performance。

Ray~\citep{Ray} 与 Kubernetes~\citep{Kubernetes} 在 cluster 或 replica 粒度管理 resource allocation、worker
placement、scaling 和 lifecycle。Ray 调度 distributed task 与 actor，Kubernetes 调度并维护 container。它们可作为
\sys{} 的外层 deployment layer，但不会原生维护 prefix-level KV ownership、依据 contiguous ready-prefix chain
路由 request，或准入 transactional KV movement。因此，\sys{} 与 cluster orchestration 互补：orchestrator 管理
replica lifecycle，\sys{} 管理一个 serving group 内的 request 与 KV placement。
...
比较并不表示 \sys{} 替代 durable 或 cross-node cache system，而是说明在所列系统中，\sys{}
...
\label{tab:related-principles}\setlength{\tabcolsep}{2pt}
\begin{tabularx}{\linewidth}{@{}L{0.15\linewidth}Y Y@{}}\toprule
系统 & 主要范围 & Locality-aware request placement\\\midrule
Mooncake Store~\citep{MooncakeStore} & Cluster-wide agentic prefix reuse & 否：提供 cache-location information 给 scheduling 来作 metadata lookup，但不路由 requests to the owner by itself 或 联合考虑 locality、load、routing 与 transfer admission。\\
PegaFlow~\citep{PegaFlow} & External、process-independent KV lifetime/sharing & 否：vLLM 保留 request scheduling，global index 只定位 remote hit\\
Aqua~\citep{Aqua} & GPU-memory burst 下 responsive preemption & 否：无 content-addressed prefix routing\\
\sys{} & Data-parallel GPU replica 间的 KV locality/fluidity & 是：最长 contiguous prefix 与 load/effective capacity 联合评分\\\bottomrule
\end{tabularx}

\vspace{0.55em}
\begin{tabularx}{\linewidth}{@{}L{0.15\linewidth}Y L{0.14\linewidth}Y@{}}\toprule
系统 & Fluidity mechanism/path & Durable/multi-tier pool & Coupled route-transfer admission\\\midrule
Mooncake Store~\citep{MooncakeStore} & GPU HBM 与 distributed cache worker 间的 asynchronous GPUDirect RDMA & 是：DRAM/SSD & 否：无公开模型比较 recomputation 与 transfer cost\\
PegaFlow~\citep{PegaFlow} & Local CUDA IPC、cross-node one-sided RDMA；cold block 使用 SSD & 是：pinned DRAM、remote DRAM、SSD & 否：TinyLFU 管理 cache admission，不在 routing/transfer 间选择\\
Aqua~\citep{Aqua} & 经 NVLink/NVSwitch 将 inference state page 到 spare GPU HBM & 否：volatile GPU HBM & 否：基于 memory pressure，而非 expected future prefix reuse\\
\sys{} & 在 configured NVLink pair 的 peer HBM 间 copy/move & 否：volatile GPU HBM & 是：先 route，再基于 measured cost 与 expected reuse 准入 transfer\\\bottomrule
\end{tabularx}\end{table}
...
在两个模型的长共享前缀下显示稳定 routing gain，验证了 load skew 中的 background placement 及 
memory skew 中 foreground offloading 的 source-block release 的功能，但 completed movement 本身不保证更低 E2E latency 或更高
throughput。

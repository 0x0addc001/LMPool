...
端到端实验验证了 background placement 以及 foreground offloading 的实现，
...
在 load skew 下执行，它们带来什么 serving trade-off？(Q4) Foreground offloading 是否能在 memory skew
...
\subsection{Q1：KV-aware routing 是否在长共享前缀下改善有效 prefix reuse 和 service？}
...
在 load skew 下执行，它们带来什么 serving trade-off？}
...
下安全释放 source capacity？}

\paragraph{0.6B 验证 offloading。} Memory skew 中，0.6B full-system 结果报告 35 个释放的

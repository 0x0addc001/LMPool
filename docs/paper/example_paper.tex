...
\begin{abstract}
Prefix caching avoids repeated prefill computation, but independent KV caches in data-parallel LLM replicas can place reusable state on a GPU other than the selected worker, while strict owner routing can overload that worker. I present \sys{}, a Mini-vLLM prototype that combines KV-aware routing for cache locality with direct NVLink transfer for cache fluidity when placement no longer matches demand. The router balances contiguous ready-prefix reuse against queueing and capacity costs, while a pair- and payload-aware cost model admits copy or move operations only when predicted saved prefill time covers the transfer cost; a versioned transaction protocol keeps incomplete replicas from routing decisions. On six RTX 3090 GPUs and Qwen3-0.6B and Qwen3-1.7B, 5x shared-prefix routing improves throughput by 11.8\% and 21.3\%, reduces mean TTFT by 18.6\% and 25.2\%, and reduces uncached prefill tokens by 59.9\% and 61.0\% relative to round-robin data parallelism. The packed all-layer K/V microbenchmark validates pair-specific 1--64-block transfer profiles, and the end-to-end results validate background placement and 0.6B move-style source release while showing that completed transfer does not by itself guarantee lower E2E latency or higher throughput; source code is available at \url{https://github.com/0x0addc001/LMPool}.
\end{abstract}
]
...
The repository provides focused tests for each engine module rather than relying only on successful benchmark completion. The current CPU test run includes 190 passing tests and one skipped hardware-gated test. Table~\ref{tab:tests} summarizes the coverage.
...
The evaluation addresses four questions: (Q1) Does KV-aware routing improve useful prefix reuse and service under long shared prefixes? (Q2) What latency and bandwidth does the transfer data path provide? (Q3) Do background copies and placement leases execute under load skew, and what serving trade-off do they create? (Q4) Can foreground move safely release source capacity under memory skew? Q1 is the primary end-to-end performance claim. Q2--Q4 establish data-path correctness, protocol behavior, and the boundary at which completed transfer does not yield an incremental serving gain.
...
Every JSON artifact records the command, resolved model configuration, Git state, values from individual trials, 95\% confidence intervals, and per-rank statistics. Each scenario runs in fresh processes, so cache contents do not persist across scenarios. The complete artifact set corresponds to the paper batch dated 20260727T231622Z.
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}L{0.13\textwidth}rL{0.29\textwidth}rrrrL{0.19\textwidth}@{}}
    \toprule
    Workload & Requests & Prefix Construction & Prompt Tokens & Req. Share & Token Share & Window & Purpose \\
    \midrule
    Routing & 192 & 16 recurring groups; first request in each group is cold; shared prefix length is 1x, 3x, or 5x & 365,880 / 1,084,728 / 1,803,576 & 91.67\% & 86.20\% / 91.38\% / 89.93\% & 16 & Isolate locality-aware routing. \\
    Load skew & 192 & Three long hot groups; three warm-ups followed by 189 hot reuses & 1,444,344 & 98.44\% & 97.15\% & 96 & Test background copy and placement leases. \\
    Memory skew & 72 & Six session warm-ups, 30 anchor/cold pressure requests, and 36 session reuses & 486,554 & 91.67\% & 72.29\% & 48 & Test move-style source-capacity release. \\
    \bottomrule
  \end{tabular}
\end{table*}
...
\subsection{Q1: Routing Improves Long-Prefix Service}
...
\subsection{Q3: Background Copy And Placement Leases}
...
\subsection{Q4: Capacity Offload Boundary}

\paragraph{Move-Style Release Is Verified At 0.6B.} In memory skew, the 0.6B full-system result reports 35 released source blocks, two copy operations, and a true offload verification flag. Transfer-only remains close to round robin, with 68.47 versus 68.75~tok/s and 714 versus 716~ms TTFT, so movement alone does not remove enough work to improve service. Routing is the dominant improvement: it raises throughput from 68.75 to 80.30~tok/s and reduces TTFT from 716 to 333~ms by selecting existing local prefix chains. Adding the move path yields 80.11~tok/s and 329~ms, which is statistically close to routing-only. Thus the experiment verifies a safe capacity-release transaction without demonstrating an incremental throughput gain.

\paragraph{The 1.7B Run Does Not Close The Offload Claim.} The 1.7B result records moved blocks but does not satisfy the complete offload verification predicate. Its full-system throughput and latency are close to routing-only, with confidence intervals that overlap. The paper therefore uses memory skew as an implementation and safety result, not as evidence that transfer-out improves serving performance. This negative boundary is important because memory pressure alone does not ensure a reusable, feasible, and sufficiently valuable transfer plan.

\section{Discussion, Limitations, and Future Work}
\label{sec:limitations}

The synthetic phase structure supports causal attribution, but cannot replace the profiling of production traces. The background-placement workload uses deterministic prompt identities and a fixed replay order, rather than an evaluated online forecast. Future evaluation should replay a published or anonymized trace and report prefix popularity, reuse distance, prompt and decode distributions, transitions in session affinity, forecast accuracy, and invalid-copy rate before selecting cache policies.
...
Future work should evaluate an online predictor on public or anonymized multi-turn traces, including precision, recall, lead time, and invalid-copy rate. The evaluation should compare against at least one production serving engine under the same model, arrival process, and KV budget, and should vary model size, KV geometry, block size, and GPU count. The transfer study should report held-out prediction error and ablate the piecewise-linear profile, EWMA correction, safety ratio, and no-gate policy. A production deployment would also require replicated control metadata and launcher failover rather than the current single-control-process recovery model.
...
\sys{} coordinates physically local KV caches through global routing for cache locality and pair-local NVLink transfer for cache fluidity only when the expected benefit exceeds the cost. The implementation separates an authoritative control process from per-GPU data processes and combines versioned metadata with transactional block movement over configured direct pairs. The experiments show stable routing gains for long shared prefixes across both models, validate background copy plus lease routing in load skew and source-block release in 0.6B memory skew, and establish the current boundary: completed movement does not by itself guarantee lower E2E latency or higher throughput.

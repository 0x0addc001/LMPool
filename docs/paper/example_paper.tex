...
The evaluation uses six 24~GiB GeForce RTX 3090 GPUs selected from a host with seven GPUs. The topology reports NV4 connectivity for physical pairs $(0,1)$, $(3,4)$, and $(5,6)$; the benchmark processes observe these GPUs as logical pairs $(0,1)$, $(2,3)$, and $(4,5)$. I evaluate Qwen3-0.6B and Qwen3-1.7B using BF16. Both models contain 28 layers, eight KV heads, and a head dimension of 128, which yields the same 114,688 KV bytes per token. The KV block size is 256 tokens. Each system experiment uses a fixed KV block budget per worker and five repetitions with seed~0. EOS termination is disabled, so every request generates 64 output tokens. Goodput includes output tokens from requests with E2E latency no greater than 10~s.

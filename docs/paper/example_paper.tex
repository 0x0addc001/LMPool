...
Background placement handles predictable future reuse rather than an immediate allocation failure. The control plane tracks complete-chain frequency and can copy a hot chain to an idle NVLink partner. It batches compatible candidates, caps blocks per plan, and creates a placement lease only after publication. Later matching requests are striped between source and replica, coupling speculative placement to routing rather than merely duplicating data. A hotness threshold, load-skew threshold, expected-reuse estimate, pair cooldown, and idle-pressure guard reject low-value work.
...
\textbf{Execute.} Source and destination issue matching NCCL point-to-point operations. All selected layers, K/V tensors, and blocks are packed into a contiguous payload per direction to amortize communicator and launch overhead. Pair communicators are prewarmed.
...
These values calibrate the online cost model. They establish data-path correctness and attainable batch bandwidth, but do not by themselves imply an end-to-end gain: useful future reuse must exceed this latency plus runtime interference.

...
For each candidate, GlobalScheduler finds the longest contiguous ready prefix chain and estimates
\begin{equation}
 C(g) = C_{\mathrm{prefill}}(g) + C_{\mathrm{queue}}(g)
       + C_{\mathrm{reclaim}}(g) - C_{\mathrm{locality}}(g).
\end{equation}
$C_{\mathrm{prefill}}$ grows with unmatched prompt tokens, $C_{\mathrm{queue}}$ includes waiting tokens and decode-weighted running work, and $C_{\mathrm{reclaim}}$ prices a real block shortage. $C_{\mathrm{locality}}$ rewards matched blocks using the topology affinity. Free capacity is a tie breaker rather than a substitute for load. Effective capacity includes dependency-safe reclaimable cached leaves, not pinned live decode blocks.

The lowest-cost feasible candidate wins. A prefix owner may be bypassed when its sequence pressure exceeds its NVLink partner by a configured ratio and the extra prefill cost remains bounded. On a miss, the router chooses the candidate with the lowest projected load and sufficient effective capacity. This policy keeps cache locality from overwhelming data-parallel utilization.

\subsection{Local Allocation and Reclamation}

The Local Block Manager first references a matching ready local block and increments its reference count. Otherwise, it allocates from a deque of free block identifiers and publishes a complete block only after the model has written its KV data. Completed prefixes remain cacheable at reference count zero. Reclamation operates on dependency-safe leaves so removing an ancestor cannot orphan a retained descendant. Coldness combines access frequency and recency; live blocks with positive references are not move-eviction victims, although they may be copied when replication is profitable.
...
Admission compares estimated transfer cost with estimated saved prefill work:
\begin{align}
 T_{\mathrm{xfer}} &= T_{\mathrm{fixed}} + \frac{B}{\widehat{BW}}\,I,\\
 T_{\mathrm{save}} &= N_{\mathrm{reuse}}\,N_{\mathrm{cached}}\,t_{\mathrm{prefill}}.
\end{align}
Here $B$ is payload bytes, $\widehat{BW}$ is an EWMA initialized from the transfer microbenchmark, and $I$ prices memory-system interference. A plan is admitted only if it has a real shortage, executable source and destination capacity, a minimum block batch, and $T_{\mathrm{save}}/T_{\mathrm{xfer}}$ exceeds a configured threshold. Repeated failures are cooled down by reason.
...
The current study uses deterministic synthetic prompts, not a public conversational trace. Each prompt applies the Qwen chat template to a repeated instruction body and group-specific suffix. This makes the amount and ownership of reusable KV explicit. Table~\ref{tab:datasets} profiles the tokenized inputs and phase construction. Dataset-level reporting is important because request hit rate alone hides how many prompt tokens are actually reused.

\begin{table*}[t]
  \centering
  \caption{Synthetic workload dataset profiles. Prompt-token counts are tokenizer outputs for one scenario trial and are identical across the two models.}
  \label{tab:datasets}
  \footnotesize
  \setlength{\tabcolsep}{5pt}
  \begin{tabular}{@{}lrrrrrp{0.31\textwidth}@{}}
    \toprule
    Workload & Requests & Prefix groups & Prompt tokens & Output tokens & Submit window & Construction and purpose \\
    \midrule
    Locality/routing & 192 & 16 & 365,880 & 12,288 & 16 & Multiple long shared prefixes interleaved across requests; isolates locality-aware routing with transfer disabled. \\
    Load skew & 192 & hot+cold & 320,232 & 12,288 & 16 & 75\% of requests target a hot family and 25\% use cold groups; tests whether load pricing avoids a cache-owner hotspot. \\
    Memory skew & 128 & 15 & 214,064 & 8,192 & 16 & Warmup, pressure, and reuse phases under a 64-block budget; induces foreground shortage and tests transfer admission. \\
    Session handoff & 128 & 32 & 244,048 & 8,192 & 64 & 32 warmup sessions create prefixes, then 96 reuse requests shift to each owner's NVLink partner; tests predictive copy and route coupling. \\
    \bottomrule
  \end{tabular}
\end{table*}

...
最佳简短回答：LMPool 先用全局元数据决定“请求去哪”，再由目标 worker 决定“本地复用还是分配”；只有真实 shortage 或已获准的预测性 placement 才进入 transfer。KV tensor 始终在 Model Runner 之间传输，control plane 只处理 metadata 和 transaction phase。
...
入口请求还没有 initial GPU，因此可以在所有健康 replica 中选择最低预计成本。物理 transfer 必须有明确 source 和 destination，而且当前原型只支持配置或发现出的 direct NVLink pair。全局 routing 不意味着全局任意 GPU 间都能直接搬 KV。
...
## 15. 为什么测量 1/2/4/8/16/32/64 blocks？
...
它使用 benchmark 提供的 exact queued-request demand，可以视为受控的 accurate forecast。它不是对线上 predictor 的评估。最佳回答是：该 workload 隔离验证 placement、transaction 和 lease routing 在机会存在时是否有效；在线预测的 precision、recall、lead time 和 invalid-copy rate 仍是 future work。

## 24. 与 vLLM 或 SGLang 相比，LMPool 的新意是什么？

vLLM PagedAttention 和 SGLang RadixAttention主要解决单个 engine 内的 block allocation 和 prefix reuse。LMPool 研究 data-parallel replica 之间的 admission-time placement：全局路由选择在哪个独立 allocator 上执行，并在 direct NVLink pair 上以成本门控方式复制或移动完整 prefix chain。
...
当前工作优先构造可控 workload，用于区分 routing、transfer、memory pressure 和 naturally replicated locality。代价是 external validity 不足。最佳回答不是回避，而是明确：论文结论是 prototype mechanism study；下一步是 public multi-turn trace、online forecast 和相同 KV budget 下的 production engine baseline。
...
优先补成本模型与 forecast 的联合有效性实验：在一个公开多轮 trace 上实现在线 demand estimate，同时记录每个 candidate plan 的 predicted cost、observed cost 和后续实际 reuse。报告 prediction precision/recall、invalid-copy rate、cost MAE/P95 error，以及 no-transfer、always-transfer、profile-only、profile+EWMA 的 throughput/TTFT。该实验直接回答当前最强的审稿攻击：系统能否在未知流量中正确发现并选择有利 transfer。

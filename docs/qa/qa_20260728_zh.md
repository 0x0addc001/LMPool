...
这里的额外 recomputation 是 spill rank 相比 owner 多执行的 missing-prefill 和 reclaim work，连同两边的 queue work 一起形成 route cost。它不把 transfer cost 直接加进 spill 的 route cost，因为“立即把本次请求 spill 后在目标重算”和“先复制 KV 再服务未来请求”是两个不同动作。Global Scheduler 会先单独判断 routing-guided copy 是否开启且其预计复用收益能覆盖实测 transfer cost；只有该 copy 不成立时，才允许在 `owner_spill_max_extra_cost` 范围内直接 spill 并重算。当前论文配置关闭普通 routing-guided speculative copy：foreground transfer 由真实 block shortage 触发，background transfer 由 handoff forecast 触发。因此不能把 bounded spill 描述为隐式执行了一次 transfer。
...
## 9. KV block 在 Block Manager 之间传输吗？
...
### Follow-up：KV cache 是怎么 version 的？
...
## 13. 40 ms fixed term 到底是什么？

最终 paper transfer workload 的命令确实设置了 `--foreground-transfer-fixed-latency-ms 40.0`。它不是由一个可独立复核的 standalone Qwen3-0.6B pilot 拟合出的精确 residual。正确表述是：40 ms 是手工选择的保守 cold-start prior，用于覆盖 idle microbenchmark 没有包含的调度、协调、NCCL 排队、destination publication 和 block registration。

Payload-dependent cost 来自每个 pair 的实测 P95 profile；运行时 source 和 dispatch-to-publish observation 会按 pair 和 size bucket 更新非负 EWMA。Admission 取静态先验和在线观测的最大值。当前缺少 40 ms sensitivity 和 held-out prediction error，这是论文明确承认的 limitation。
...
“缺少 40 ms sensitivity”是指尚未系统地比较例如 0/10/20/40/60 ms 时 plan 接受率、throughput 和 latency 如何变化。“缺少 held-out prediction error”是指尚未用一批运行校准 profile/EWMA，再在完全独立的 plan 上报告 predicted versus observed cost 的 MAE、P95 error 和 underprediction rate。因此，多尺寸数据路径是实测的，但固定截距和完整 serving-cost 预测仍缺少独立验证。
...
## 18. Placement lease 是什么？Routing 是否会被 transfer 影响？

会，而且这是设计目标。Destination publish 后，Global Block Manager 的 page table 出现新 replica。Placement lease 随后给 source 和 destination 分配 request quota，使 routing 能利用复制出的 KV。没有 publish 前不可见，没有 lease 时普通 cost-based route 仍可选择副本。

Placement lease 不是 GPU memory lock，也不是 KV ownership 的永久转移。它是 control plane 在一次 forecast-driven background copy 成功后，为某个 prefix leaf 创建的短期路由配额记录。记录包含 source GPU、replica GPU、两侧剩余 request quota、下一次优先选择哪一侧以及所属 NVLink pair。后续匹配该 prefix 的请求在进入普通 route-cost 计算前，先按 source/replica 交替消费这些配额；每次仍重新检查目标上是否存在连续 ready chain 且容量是否可用。配额耗尽后 lease 被删除，routing 回到普通成本模型。

Lease 解决的是“复制完成但请求仍集中在原 owner”问题。Transfer 改变 Global Block Manager 可见的 replica 集合，lease 则把已知的 forecast demand 分摊到 source 和 destination，使复制出的 KV 真正转化为并行服务能力。

### Follow-up：Transfer 是否会被 routing 影响？

会。Routing 会改变每个 rank 的请求、prefix access、queue pressure、free capacity 和已有 replica 分布，这些状态直接影响后续 transfer 的候选链、source/destination 和预期收益。若 routing 已经把请求送到可复用 owner 且负载可接受，就没有必要 transfer；若 forecast 表明未来请求会转移到 partner，或真实 shortage 发生，transfer 才可能有价值。普通 route hit 和 bounded spill 本身不会无条件触发 transfer。当前论文配置中，foreground transfer 由真实 shortage 触发，positive background transfer 由 benchmark 提供的 handoff forecast 触发。

## 19. 为什么 transfer-only cached-token ratio 很高但 throughput 没提高？

在 session handoff 中，transfer-only 和完整 LMPool 都达到 70.5% cached-token ratio，但 transfer-only 保留 3.3 的 rank request ratio，GPU utilization 仍约 31--33%。它减少了 cold prefill，却没有把请求分配到新增副本。完整 LMPool 通过 placement-lease routing 将 request ratio 降至 1.8，并将 mean GPU utilization 提升到约 50--52%，因此 throughput 才提高。
...
如果 routing 已经能把请求送到 prefix owner，并且 owner 没有严重过载，那么 transfer 的增量空间很小。Qwen3-0.6B session handoff 中，routing-only 和完整系统的 mean-E2E 95% confidence intervals 重叠。论文因此只主张 transfer 对 throughput 和较高 compute-intensity 模型的增量证据更强，不主张所有 metric 都显著优于 routing-only。

## 21. 为什么 load-skew workload 没有体现 transfer 优势？

该 workload 的 144 个 hot requests 被 round robin 平均分到六个 worker，每个 worker 很快自然缓存同一个 hot prefix。Baseline cached-token ratio 已约 85.9%，每个 rank 正好 32 个请求。此时 KV placement 已经足够均匀，routing 和 transfer 没有额外可消除的工作。
...
## 23. Session-handoff workload 是否使用 oracle？

它使用 benchmark 提供的 exact queued-request demand，可以视为受控的 accurate forecast。它不是对线上 predictor 的评估。该 workload 隔离验证 placement、transaction 和 lease routing 在机会存在时是否有效；在线预测的 precision、recall、lead time 和 invalid-copy rate 仍是 future work。

Oracle 指实验向系统提供了真实运行中通常无法提前完整知道的信息。这里 benchmark 已经生成全部 reuse-phase prompts，因此可以精确计算每个 prefix 在接下来 96 个 queued requests 中会出现多少次，并在 phase boundary 一次性提交给 control plane。这相当于 perfect demand oracle。它用于回答“如果未来需求判断正确，transfer 和 routing 机制能否兑现收益”，不能回答“线上 predictor 能否准确预测”。

Lease routing 是 oracle/forecast copy 成功后的短期配额路由：control plane 为 source 与 destination 分配 future-demand quota，并交替把匹配请求送到两边；每次仍验证 ready prefix 和 capacity。它不同于普通最低成本 routing，因为 lease 在已知 demand 范围内显式保证新 replica 被使用，配额耗尽后自动失效。
...
在当前结果中，Qwen3-1.7B 通常更能体现“避免 prefill”的价值。Routing workload 中，两者都把 cached-token ratio 从约 44% 提升到约 72%，但 Qwen3-0.6B 的 mean TTFT 反而上升 7.4%，Qwen3-1.7B 则下降 10.2%。Session handoff 中，完整 LMPool 相对 round robin 的 throughput 提升分别为 4.4% 和 7.4%，TTFT 降低分别为 32.4% 和 40.4%，E2E 降低分别为 9.3% 和 12.9%。原因是两者 KV geometry 相同、同一 plan 的 transfer bytes 基本相同，但 1.7B 每个 uncached prefill token 的计算更贵，因此 saved compute / transfer cost 比例更高。
...
不是单独越高越好。高 utilization 只有在 throughput、latency 和 goodput 同时改善时才说明资源被有效利用。Session handoff 中，LMPool 的 utilization 从约 32--35% 提升到 50--52%，同时 throughput 和 latency 改善，因此可解释为负载分布更有效。若 utilization 提高但 latency 恶化，可能只是拥塞。
...
这里的 model warm-up 是正式 serving 计时前运行少量合成 prefill/decode，使 CUDA context、模型权重、KV cache allocation、kernel path 和 NCCL communicator 完成初始化，避免把一次性启动开销计入每请求 latency。它不同于 session-handoff workload 的 warm-up phase。后者位于正式 workload 内并计入 phase/aggregate metrics：benchmark 把前 32 个请求固定送到每个 NVLink pair 的 source 侧，为 32 个 prefix group 各创建一个完整长前缀；等待这些请求全部完成后，才进行 background placement 并进入 96-request reuse phase。
...
更广泛的收获是建立以可测量代价分析 tradeoff 的能力：locality 与负载均衡、transfer 与 recomputation、较大 block 的批量效率与内部碎片、保守 admission 的安全性与 missed opportunity、强一致性保护与控制面开销、集中式元数据的简单性与高可用性，以及 exact forecast 的机制隔离能力与生产真实性。真正的系统优化不是让单个指标最大，而是在明确 correctness boundary 后，用 workload、成本模型、消融和 negative result 判断哪一侧更值得。

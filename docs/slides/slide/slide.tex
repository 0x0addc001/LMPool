...
\begin{frame}{Transfer: Mechanism Evidence and Boundary}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \end{column}
    \begin{column}{0.48\textwidth}\small
      \caution{Memory skew:} Releasing 35 source blocks verifies a safe capacity-release transaction, but does not demonstrate an incremental throughput gain on its own.
    \end{column}
  \end{columns}
\end{frame}

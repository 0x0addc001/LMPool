...
LMPool 将集群内多张 GPU 的 HBM 抽象为一个逻辑统一的全局 KV Cache 池。它在 Mini-vLLM 的 Paged Attention 基础上，扩展了跨 GPU 的块级前缀感知路由和冷热感知驱逐。
...
3. **冷热感知**：LRU 驱逐 + 拓扑优先的 swap（NVLink > PIX > NODE）
...
#### 3.2.3 冷热感知驱逐（`select_eviction_candidates`）
...
4. 若远端也完全为空（无块可选），则直接覆盖写入——目标块计数不变，旧数据在传输到达时被覆盖

`_get_target_gpu_order(gpu_id)` 构建顺序如下：

```python
ordered = []
# 第一级：NVLink 直连伙伴（带宽最高）
...
内部计数器递增，每 `sync_interval`（默认 10）轮调度后调用一次 `broadcast_page_table()`。目前在 `Scheduler` 中被注释掉，重新启用是激活真正跨 GPU 前缀复用的首要步骤。

#### 3.2.6 Master 故障迁移

`GlobalBlockManager` 包含完整的故障检测骨架，但尚未在生产路径中启用：
...
| 功能              | 状态       | 说明                                        |
| ----------------- | ---------- | ------------------------------------------- |
| 对等式多 GPU 推理 | ✅ 完成     | 两个 rank 独立调度、执行、采样              |
| 跨 GPU 序列路由   | ✅ 完成     | `route_sequence` 正常工作                   |
| 全局页表同步      | ❌ 未启用   | `maybe_sync` 被注释，各 rank 页表独立       |
| `swap_out`        | ✅ 已触发   | 日志确认执行                                |
| `swap_in`         | 🔄 进行中   | 端到端待验证                                |
| 前缀复用（去重）  | ❌ 未生效   | 页表不同步 + 远程分配不复用已有块           |
| 拓扑感知驱逐      | ✅ 代码就绪 | `select_eviction_candidates` 实现了三级策略 |
| RadixTree 前缀树  | ❌ 未实现   | 当前哈希链足够，后续优化                    |

### 5.2 未来工作

1. **恢复 `maybe_sync()`**：这是整条链路的关键缺口——让 `lookup_prefix` 能查到跨 GPU 的前缀命中
2. **验证 swap 端到端**：构造两个 rank 之间的 NCCL 收发场景，确认无死锁且数据正确
3. **实现前缀块复用**：修改 `BlockManager.allocate` 接受 `BlockLocation` hint，直接引用已有物理块而非重新分配，关闭前缀复用的最后缺口
4. **构造高并发长前缀 benchmark**：量化 cache hit rate 和 TTFT 在共享前缀场景下的提升
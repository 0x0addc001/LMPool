...
对于 `N` 张 GPU，运行时包含一个用户/launcher 进程、一个独立控制面进程，以及 `N` 个完全对称的数据面进程；rank 0 也运行在单独的 worker 进程中。
...
Foreground transfer 只申请实际 shortage，而不是整条 sequence 的 block 数量。Background placement 会为未来复用批量复制热 prefix 链。两者都必须通过源块有效性、目标容量、最小批量，以及预计节省 prefill 时间与传输成本比值等门控条件。
...
`kv_transfer.py` 内部仍保留 legacy 函数名 `swap_out`/`swap_in`；对外文档统一使用 **transfer**，因为该操作既可以是 copy，也可以是 move，并不表示存在 CPU swap 层。

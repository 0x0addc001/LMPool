...
为了隔离验证 transfer，`session-handoff` 使用等长的 warmup/reuse 两阶段：前缀只在 source
侧建立，随后同一批会话固定在 NVLink partner 继续。`--handoff-prefix-groups` 控制跨 pair
的独立会话数，避免 baseline 在被测 handoff 前自行预热目标端。
...
一次 prepare/execute 协议和一个连续 KV payload。phase 会等待已接受的放置计划完成，
...
受预测需求约束的 placement lease。由于 source 已经完成并排空 warmup 阶段，预测到的 reuse
请求会成批留在 replica，除非目标端 KV 容量不足，从而避免逐请求重新切碎 decode batch。
